...
describe("extractFigmaSignals", () => {
  it("detects design URL and file key from US-PLG-005-style markdown", () => {
    const { figmaReadRequired, figmaFileKeys, figmaUrls } = extractFigmaSignals(US_PLG_005_SNIPPET);
...
`;
  ...
  it("returns false when no Figma URLs", () => {
    const { figmaReadRequired, figmaFileKeys, figmaUrls } = extractFigmaSignals("No design here.");
