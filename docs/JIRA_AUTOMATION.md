...
2. **Action:** Send **web request** (outbound HTTP).
   ...
   - **Body (JSON):**
     ...
     Optional **`project`**: pack id for this run (e.g. **`EPMRPP`** → **`projects/EPMRPP/`** when present). Omit for generic-only output. Legacy **`project_pack`** in **`client_payload`** is still accepted.
