...
| `FIGMA_API_KEY` | No* | Figma API token for **[acuvity/mcp-server-figma](https://hub.docker.com/r/acuvity/mcp-server-figma)** (`FIGMA_API_KEY` in rendered **`mcp.json`** env). Optional for stories **without** Figma links. *Required when **`scripts/extract-requirement-signals.js`** detects any **`figma.com`** URL in merged requirements—otherwise extract fails before the agent runs. When set, the workflow runs **`scripts/verify-figma-access.js`** (Figma **`GET /v1/me`**) before MCP config generation; **if that check fails, the workflow stops** (no MCP render or agent steps). File-level Figma reads happen later via MCP (`figmaReadRequired`). |
| `GITLAB_PERSONAL_ACCESS_TOKEN` | No* | GitLab **personal access token** for **[mcp/gitlab](https://hub.docker.com/r/mcp/gitlab)** (`GITLAB_PERSONAL_ACCESS_TOKEN` in rendered **`mcp.json`**) and for **`scripts/extract-requirement-signals.js`**. Pair with repository variable **`GITLAB_API_URL`**. *Required when the story Jira **description** contains any **`git.epam.com`** `/-/blob/` link—the extract step fails before the agent runs if credentials are missing or any linked file cannot be read. |
...
| `GITLAB_API_URL` | No* | GitLab **HTTP API base URL** (e.g. `https://gitlab.com/api/v4` or `https://git.epam.com/api/v4`). Passed to the GitLab MCP container and **`extract-requirement-signals.js`**. *Required with **`GITLAB_PERSONAL_ACCESS_TOKEN`** when the Jira **description** includes **`git.epam.com`** `/-/blob/` URLs. |
