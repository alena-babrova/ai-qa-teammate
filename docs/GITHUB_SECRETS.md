...
| `GITLAB_PERSONAL_ACCESS_TOKEN` | No* | GitLab **personal access token** for **[mcp/gitlab](https://hub.docker.com/r/mcp/gitlab)** (`GITLAB_PERSONAL_ACCESS_TOKEN` in rendered **`mcp.json`**) and for **`scripts/extract-requirement-signals.js`**. Pair with repository variable **`GITLAB_API_URL`**. *Required when the story Jira **description** is **only** a **`git.epam.com`** requirements link—otherwise the extract step fails before the agent runs (GA/UI AC live in GitLab). |
...
| `GITLAB_API_URL` | No* | GitLab **HTTP API base URL** (e.g. `https://gitlab.com/api/v4` or `https://git.epam.com/api/v4`). Passed to the GitLab MCP container and **`extract-requirement-signals.js`**. *Required with **`GITLAB_PERSONAL_ACCESS_TOKEN`** when stories link requirements only on **`git.epam.com`**. |
