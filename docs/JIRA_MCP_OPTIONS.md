...
A self-hosted Jira host (for example `jira.example.com`) is usually **Jira Data Center** (or Server). **Jira Cloud–only** MCPs use different auth and APIs and may **not work** against DC.

This pipeline only ever **reads** from Jira, so the Atlassian MCP server is started with **`READ_ONLY_MODE=true`**: whichever server you choose, it should expose read tools only.
...
- **Tool names** are upstream’s (e.g. **`jira_get_issue`**, **`jira_search`**). Repo rules refer to **`get_issue`** / **`jira_getIssue`** as generic equivalents.
...
## Usually not suitable for self-hosted DC without verification
